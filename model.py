...
from localLogger import update_predict_log, update_train_log
...
MODEL_DIR = "models"
MODEL_VERSION = 0.2
MODEL_VERSION_NOTE = "SGDRegression"
...
def _model_train(df, tag, test=False):
    ...
    param_grid_sgd = {
        'sgd__l1_ratio': [.1, .5, .7, .9, .95, .99, 1]
    }

    pipe_rf = Pipeline(steps=[('scaler', StandardScaler()),
                              ('rf', RandomForestRegressor())])

    pipe_sgd = Pipeline(steps=[('scaler', StandardScaler()), ('sgd',
                                                              SGDRegressor(penalty='elasticnet'))])

    # grid = GridSearchCV(pipe_rf, param_grid=param_grid_rf, cv=5, iid=False, n_jobs=-1)
    grid = GridSearchCV(pipe_sgd, param_grid=param_grid_sgd, cv=5, iid=False, n_jobs=-1)
    ...
    grid.fit(X, y)
    model_name = re.sub("solution_guidance", "_", str(MODEL_VERSION))
    if test:
