...
def update_train_log(country, dates, eval_test, runtime, MODEL_VERSION, MODEL_VERSION_NOTE, test=False):
    """
    ...
    header = ['unique_id', 'timestamp', 'country', '(from_date, to_date)', 'eval_test', 'model_version',
    ...
    with open(logfile, 'a') as csvfile:
        ...
        to_write = map(str, [uuid.uuid4(), time.time(), country, dates, eval_test,
...
def update_predict_log(country, y_pred, y_proba, target_date, runtime, MODEL_VERSION, test=False):
    ...
    with open(logfile, 'a') as csvfile:
        ...
        to_write = map(str, [uuid.uuid4(), country, time.time(), y_pred, y_proba, target_date,
...
if __name__ == "__main__":
    ...
    ## train logger
    update_train_log('hong_kong', str((100, 10)), "{'rmse':0.5}", "00:00:01",
